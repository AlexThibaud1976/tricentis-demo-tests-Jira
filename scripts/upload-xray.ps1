param(
  ...
  try {
    $authResponse = Invoke-RestMethod -Uri "https://$XrayEndpoint/api/v2/authenticate" -Method POST -ContentType "application/json" -Body $authBody -ErrorAction Stop
  } catch {
  ...
  try {
    $response = Invoke-RestMethod -Uri "https://$XrayEndpoint/api/v2/import/execution/junit?projectKey=$JiraProjectKey&testPlanKey=$IssueKey&testEnvironments=$DeviceName" -Method POST -ContentType "text/xml" -Headers @{ "Authorization" = "Bearer $token" } -Body $junitContent -ErrorAction Stop
  } catch {
